...
def create_location_map(locations):
    ...
    # Calculate center point from location history
    ...
    for loc in locations:
        folium.Marker(
            ...
            popup=f"Time: {loc['timestamp'].strftime('%H:%M:%S')}<br>Address: {loc['address']}",
...
def main():
    ...
    try:
        safety_system = IntegratedDriverSafetySystem(GEMINI_API_KEY)
    except Exception as e:
    ...
    if feature_mode == "Live Monitoring":
        ...
        try:
            while True:
                ...
                # Update map
                if safety_system.metrics['location_history']:
                    map_obj = create_location_map(safety_system.metrics['location_history'])
                    if map_obj:
